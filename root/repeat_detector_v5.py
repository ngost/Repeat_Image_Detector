    ...
    def list_classify_test(self, img):
        ...
        if error_avg > 0.04:
            if error_max > 1.0:
                ...
                return [1, str(img), error_avg, error_max, time_result]
            else:
                print("general_img" + str(error_avg) + ", " + str(error_max))
                return [0, str(img), error_avg, error_max, time_result]
        else:
            print("general_img"+str(error_avg)+", "+str(error_max))
            return [0, str(img), error_avg, error_max, time_result]
                # if result2 < 0.1:
...
if __name__ == "__main__":
    ...
    #--------------from local file list directory
    file_list = os.listdir('img/chequered/')
    file_list.sort()
    print(file_list)
    ...
    for file in file_list:
        ...
        jin = Jinyoung()
        result_list = jin.list_classify_test('img/chequered/'+file)
        results.append(result_list)
        print("\n")

    f = open('chequered_surf.csv', 'w', encoding='utf-8', newline='')
    wr = csv.writer(f)
