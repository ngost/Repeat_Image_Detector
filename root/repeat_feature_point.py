...
class RepeatCheck:
    ...
    #        plt.show()
            ...
            error_avg,error_max,max_index = different(count, dist_count)
 ...
 #           print(euclidean_result)
            ...
            if max_index is not None:
                ...
                for ind in max_index:
                    i = 0
                    for eu in euclidean_result:

                        if eu<=bins[ind+1] and eu>=bins[ind-1]:
                            index_list.append(i)
                ...
                for index in index_list:
                    ...
                    point_tuple = kp[index].pt
                    point_list = list(point_tuple)
                    pointx = round(point_list[0])
                    pointy = round(point_list[1])
                    points = (pointx,pointy)

                    cv2.circle(img, points, 9, (0, 0, 255), -1)
                    repeat_kp.append(kp[index])

                #cv2.drawKeypoints(img,repeat_kp,img)
...
if __name__ == "__main__":
    ...
    #--------------local file
    file_list = os.listdir('img/zigzagged/')
    file_list.sort()
    print(file_list)
    ...
    for file in file_list:
        ...
        jin = Jinyoung()
        result_list = jin.list_classify_test('img/zigzagged/'+file)
